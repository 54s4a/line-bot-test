# -*- coding: utf-8 -*-
# ====== asaoka_ai_layers.py ======
# 相談AI「思想レイヤー分離」最小実装（読みやすい改行＆やや口語寄り／SNSドメイン対応）
# main.py（FastAPI/Flask）から generate_reply() を呼ぶだけで動作します。
...
def gen_ops(meta: Meta, text: str) -> OpsLayer:
    ...
    if meta.domain == "SNS":
        ...
        actions = [
            {
                "step": "記録化",
                "how": "URL・スクショ・相手ID・時刻・要点を1枚のメモ/表計算に集約。",
                "success": "第三者が経緯と証拠を再現できる。",
                "risk": "スクショの撮り忘れ・リンク切れ。",
                "eta": "10分"
            },
            {
                "step": "初回レス草案",
                "how": "必要な場合のみ、事実のみを125字以内で一度だけ返信。煽らない/評価語を入れない/論争を広げない。",
                "success": "誤解点の最小訂正。議論の延焼を抑制。",
                "risk": "二往復目以降の泥沼化。挑発誘発。",
                "eta": "15分"
            },
            {
                "step": "プラットフォーム運用",
                "how": "通報・ミュート・ブロック・キーワード非表示の適切な組み合わせを実施。",
                "success": "可視被害の最小化（タイムラインを守る）。",
                "risk": "過剰ブロックによる二次炎上。",
                "eta": "5分"
            },
            {
                "step": "必要ならエスカレーション",
                "how": "脅迫や個人情報晒しは法務/警察/プラットフォーム窓口へ。テンプレ報告文を使用。",
                "success": "対応の速さと証拠保全。",
                "risk": "通報先の選定ミス・証拠不足。",
                "eta": "10分"
            },
        ...
        templates = {
            "message": (
                "ご指摘ありがとうございます。事実関係は以下のとおりです。"
                "誤解を招いた点があれば修正します。追加の確認が必要な場合は、固定ポストの説明をご参照ください。"
            )
        }
    ...
    # デフォルト（その他）
    checks = [
        "相手の要件定義（目的・期日・望む結果）を確認",
        "自分の許容上限（時間・頻度）を言語化"
    ]
    actions = [
        {
            "step": "記録化",
            "how": "要件・履歴・合意条件を一箇所に集約。",
            "success": "第三者が再現可能。",
            "risk": "抜け漏れ。",
            "eta": "10分"
        },
    ...
    templates = {"message": "本件の目的と期日を確認したく、対応可能な範囲を以下に記します。必要があれば再調整しましょう。"}
    return OpsLayer(checks=checks, actions=actions, templates=templates)
